// weeks/week_mar16.js
(function () {
  const week = {
    id: "week_mar23",
    startDate: "2026-03-23",
    endDate: "2026-03-29",
    label: "Week (Mar 23–29)",
    title: "Week 8 Menu",

    menu: [
      {
        name: "Monday",
        badge: "Training Day",
        meals: [
          {
            title: "Lunch",
            text: "豉油鸡腿 + 西兰花 + 米饭 100g（鸡腿肉 150g + 西兰花 200g，≈550 kcal）"
          },
          {
            title: "Dinner",
            text: "蒸蛋 + 蒜蓉生菜 + 米饭 100g（≈450 kcal）"
          }
        ]
      },
      {
        name: "Tuesday",
        badge: "Training Day",
        meals: [
          {
            title: "Lunch",
            text: "肉末土豆：瘦肉末 120g + 土豆 200g + 米饭 100g（≈600 kcal）+ 清炒娃娃菜"
          },
          {
            title: "Dinner",
            text: "干锅花菜（少油版）：花菜 300g + 五花肉 50g + 米饭 80g（≈550 kcal）"
          }
        ]
      },
      {
        name: "Wednesday",
        badge: "Training Day",
        meals: [
          {
            title: "Lunch",
            text: "鱼香茄子（减油版）：茄子 250g + 瘦肉 80g + 米饭 100g（≈600 kcal）"
          },
          {
            title: "Dinner",
            text: "清蒸鲈鱼 + 青菜 + 米饭 80g（鲈鱼 250g + 青菜 150g，≈500 kcal）"
          }
        ]
      },
      {
        name: "Thursday",
        badge: "Training Day",
        meals: [
          {
            title: "Lunch",
            text: "可乐鸡翅（控量版）：鸡翅 250g + 白灼菜心 150g + 米饭 100g（≈650 kcal）"
          },
          {
            title: "Dinner",
            text: "西红柿鸡蛋拌面：挂面 90g + 鸡蛋 2 个 + 番茄 2 个（≈550 kcal）"
          }
        ]
      },
      {
        name: "Friday",
        badge: "Rest Day · Two Meals",
        meals: [
          {
            title: "Meal 1",
            text: "红烧肉（控量版）120g + 白灼菜心 200g + 米饭 100g（≈700 kcal）"
          },
          {
            title: "Meal 2",
            text: "蒸蛋 + 清炒娃娃菜 + 米饭 80g（≈450 kcal）"
          }
        ]
      },
      {
        name: "Saturday",
        badge: "Break Day · One Meal",
        meals: [
          {
            title: "Shared Meal",
            text: "自由一餐（≈700–900 kcal）"
          }
        ]
      },
      {
        name: "Sunday",
        badge: "Cooking Day · One Main Meal",
        meals: [
          {
            title: "Main Meal",
            text: "卤肉饭：五花肉 200g + 白灼菜心 200g + 米饭 120g + 卤蛋 1 个（≈850 kcal）"
          }
        ]
      }
    ],

    grocery: {
      proteins: [
        { key: "eggs", label: "鸡蛋 6 个｜蒸蛋 3；拌面 2；卤蛋 1" },
        { key: "ground_pork", label: "瘦肉末 200g｜肉末土豆 120g；鱼香茄子 80g" },
        { key: "pork_belly", label: "五花肉 370g｜干锅 50g；红烧肉 120g；卤肉饭 200g" },
        { key: "chicken_wings", label: "鸡翅 250g｜Thu Lunch" },
        { key: "chicken_thigh", label: "鸡腿肉 150g｜Mon Lunch" },
        { key: "fish", label: "鲈鱼 250g｜Wed Dinner" }
      ],

      veggies: [
        { key: "potato", label: "土豆 200g｜Tue Lunch" },
        { key: "cauliflower", label: "花菜 300g｜Tue Dinner" },
        { key: "eggplant", label: "茄子 250g｜Wed Lunch" },
        { key: "tomato", label: "番茄 2 个｜Thu Dinner" },
        { key: "spinach", label: "菠菜 200g｜Fri Meal 2" },
        { key: "broccoli", label: "西兰花 200g｜Mon Lunch" },
        { key: "lettuce", label: "生菜 200g｜Mon Dinner" },
        { key: "leafy_green", label: "菜心 500g｜Wed 150g；Thu 150g；Fri 200g" }
      ],

      other: [
        { key: "noodles", label: "挂面 90g（干重）｜Thu Dinner" },
        { key: "rice", label: "米（熟重）约1200g｜按菜单使用" },
        { key: "oyster_sauce", label: "蚝油（本周新增）" },
        { key: "five_spice", label: "五香粉（本周新增）" },
        { key: "soy_sauce", label: "生抽" },
        { key: "garlic", label: "蒜" }
      ]
    }
  };

  window.WEEK_REGISTRY = window.WEEK_REGISTRY || {};
  window.WEEK_REGISTRY[week.id] = week;
})();
